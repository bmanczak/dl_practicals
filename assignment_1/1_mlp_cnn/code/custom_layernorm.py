...
class CustomLayerNormAutograd(nn.Module):
    ...
    def forward(self, input):
        ...
        mu_s = torch.mean(input, axis=1, keepdim=True)  # keepdim leaves the axis dimension => enables brodcasting later
        var_s = torch.var(input, axis=1, keepdim=True)

        X_hat = (input - mu_s) / torch.sqrt(var_s + self.eps)

        out = self.gamma * X_hat + self.beta

        assert out.shape == input.shape, "Output shape does not match the input tensor!!"
        return out
...
class CustomLayerNormManualFunction(torch.autograd.Function):
    ...
    def forward(ctx, input, gamma, beta, eps=1e-5):
        ...
        mu_s = torch.mean(input, axis=1, keepdim=True)  # keepdim leaves the axis dimension => enables brodcasting later
        var_s = torch.var(input, axis=1, keepdim=True, unbiased=False)
        X_hat = (input - mu_s) / torch.sqrt(var_s + eps)
    ...
    def backward(ctx, grad_output):
        ...
        """
        S, M = grad_output.shape

        input, gamma, beta = ctx.saved_tensors
        grad_gamma = torch.einsum("si,si->i", grad_output, ctx.X_hat)
        grad_beta = torch.einsum("si->i", grad_output)  # summing over rows


        dY = grad_output * gamma
        grad_input = 1/(M *torch.sqrt(ctx.eps + ctx.var_s)) * (
                    M * dY - torch.sum(dY, axis=1, keepdim = True) - ctx.X_hat * torch.sum(dY * ctx.X_hat, axis = 1, keepdim = True))
...
class CustomLayerNormManualModule(nn.Module):
    ...
    def forward(self, input):
        ...
        assert input.shape[1] == self.n_neurons, "What you doing? Shapes do not much dummy"

        my_bn_fct = CustomLayerNormManualFunction.apply
        out = my_bn_fct(input, self.gamma, self.beta, self.eps)
        return out
